...
ImageStore.colors = ['orange', 'magenta', 'light_blue',
...
ImageStore.prototype.getImage = function(key, cb) {
  this.get(key, function(err, body) {
    if (err) return cb(err);
    cb(null, new Buffer(doc.dataURL.replace(/^data:.+,/, ""), "base64"));
  });
}
